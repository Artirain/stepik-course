...
| **Файлы** | SKILL.md + вспомогательные файлы | `.claude/agents/` или `context: fork` в Skill |
...
| **Настройка** | Разработчик создаёт SKILL.md | Автоматически или через `context: fork` / `.claude/agents/` |
...
Есть несколько способов управлять саб-агентами:

### Автоматическое создание

Claude Code сам решает, когда использовать саб-агенты. Ты можешь подтолкнуть его:
...
### Явное создание через Skills

Используй `context: fork` в front matter Skill'а, чтобы он выполнялся в отдельном подагенте:
...
Определяй собственные типы агентов в `.claude/agents/`. Встроенные типы: `Explore` (быстрый поиск), `Plan` (планирование), `general-purpose` (универсальный).

...
!!! info "Что ты узнаешь"
    - Когда срабатывает `SessionStart` и для чего он нужен
    - Как написать скрипт инициализации сессии
    - Как передать полезный контекст через stdout
...
1. Создай скрипт `session-start.sh`, который выводит текущую ветку и количество изменённых файлов.
2. Подключи его через `settings.json` к событию `SessionStart`.
3. Запусти новую сессию Claude Code и убедись, что контекст подхватился.

## Итоги

- `SessionStart` срабатывает при запуске и возобновлении сессии
- stdout скрипта передаётся Claude как полезный контекст
- Делай хук быстрым — тяжёлые операции лучше выносить в отдельные команды

...
.claude/skills/
  название-skill/
    ...
    template.md           # Шаблоны (опционально)
    examples/             # Примеры (опционально)
      sample-component.tsx
      sample-component.test.tsx
...
Вспомогательные файлы можно организовать свободно — нет жёстких требований к именованию папок. Главное — `SKILL.md` в корне папки Skill'а.
...
| Поле | Описание |
|---|---|
| `name` | Название Skill'а (человекочитаемое) |
| `description` | Описание — когда и зачем применять (рекомендуется) |
| `allowed-tools` | Список инструментов, доступных Skill'у |
| `model` | Модель для выполнения (например, `sonnet`) |
| `argument-hint` | Подсказка формата аргументов (`[файл] [формат]`) |
| `user-invocable` | Может ли пользователь вызвать через `/имя` (по умолчанию `true`) |
| `disable-model-invocation` | Запретить Claude автовызов (по умолчанию `false`) |
| `context` | Режим выполнения: `fork` — в отдельном подагенте |
| `agent` | Тип подагента при `context: fork` (`Explore`, `Plan`, `general-purpose`) |
| `hooks` | Hooks, привязанные к жизненному циклу Skill'а |

Все поля опциональны, но `description` настоятельно рекомендуется — оно помогает Claude Code понять, когда активировать Skill.
...
- Все поля front matter опциональны; `description` рекомендуется
- Новые поля: `context: fork`, `agent`, `hooks`, `disable-model-invocation`
- Инструкции пишутся как Markdown — чётко, по шагам, с примерами
- Вспомогательные файлы (шаблоны, примеры, скрипты) хранятся рядом с SKILL.md
- Claude Code использует примеры как образцы для генерации
...
<div class="quiz-block" data-quiz-id="u42-q2" data-answer="c" markdown>
  <div class="quiz-question">Для чего используется поле description в front matter?</div>
  ...
  <label><input type="radio" name="u42-q2" value="c"> Для описания, когда и зачем Claude Code должен применять Skill</label>
  <button class="quiz-btn" onclick="checkQuiz(this)">Проверить</button>
  <div class="quiz-result"></div>
</div>

...
Когда ты вызовешь `/review src/utils/parser.ts`, Claude Code получит промпт:
...
Вызов: `/explain Dependency Injection`
...
Вызов: `/migrate src/components/Header.jsx`
...
Вызов: `/api-endpoint users`

## Индексный доступ: $ARGUMENTS[N]

Помимо `$ARGUMENTS` (весь текст), доступен индексный синтаксис `$ARGUMENTS[N]` с **нулевой индексацией**:

```
/review src/app.ts strict
```

| Переменная | Значение |
|---|---|
| `$ARGUMENTS` | `src/app.ts strict` |
| `$ARGUMENTS[0]` | `src/app.ts` |
| `$ARGUMENTS[1]` | `strict` |

Позиционные переменные `$1`, `$2` — это сокращения для `$ARGUMENTS[1]`, `$ARGUMENTS[2]` (см. урок 25).

## Что если $ARGUMENTS нет в шаблоне

Если ты не используешь `$ARGUMENTS` нигде в теле команды, аргументы всё равно будут переданы — они добавятся в конец промпта как `ARGUMENTS: <значение>`.
...
<div class="quiz-block" data-quiz-id="u24-q1" data-answer="b" markdown>
  <div class="quiz-question">Что содержит переменная $ARGUMENTS при вызове `/review src/app.ts`?</div>
  <label><input type="radio" name="u24-q1" value="a"> review</label>
  <label><input type="radio" name="u24-q1" value="b"> src/app.ts</label>
  <label><input type="radio" name="u24-q1" value="c"> /review src/app.ts</label>

...
# Headless-режим (Agent SDK) — Claude Code без интерактива

!!! info "Что ты узнаешь"
    - Что такое headless-режим (Agent SDK) и зачем он нужен
...
По умолчанию Claude Code работает в интерактивном режиме — ты вводишь промпт, получаешь ответ, продолжаешь диалог. Но что, если нужно запустить Claude Code из скрипта? Или встроить его в CI/CD-пайплайн? Для этого есть headless-режим (в документации также называется **Agent SDK**).

```mermaid
graph LR
    A[Скрипт / CI] -->|claude -p 'промпт'| B[Claude Code]
    B --> C{--output-format}
    C -->|text| D[stdout текст]
    C -->|json| E[stdout JSON]
    D --> F[Процесс завершён]
    E --> F
```
...
```bash
claude -p "Проверь стиль кода" --allowed-tools Read,Grep
```

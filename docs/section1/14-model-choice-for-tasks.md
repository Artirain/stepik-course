...
Модель можно задать не только командой `/model`, но и при старте:

```bash
claude --model opus    # запустить сессию сразу на Opus
```

Это удобно, когда ты заранее знаешь, что задача сложная.

## Связь с extended thinking

На сложных задачах модель использует extended thinking — внутреннюю цепочку рассуждений. Это особенно заметно на Opus. Подробнее — в [Уроке 13](13-extended-thinking.md).
